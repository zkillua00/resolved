...
crate_sha256="979b45cfa6ec723b6f42330915a1b3769b930d02b2d505f9697f8ca602bee707"
patch_file="$project_dir/patches/gpui-0.2.2-metal-memoryless.patch"
...
temporary_dir="$(mktemp -d "${TMPDIR:-/tmp}/api-tester-gpui.XXXXXX")"
...
if ! (cd "$source_dir" && patch --batch -p1 <"$patch_file"); then
    echo "error: GPUI patch no longer applies to crates.io $crate_name $crate_version" >&2
    exit 1
fi

printf '%s\n%s\n' "$patch_sha256" "$crate_sha256" >"$source_dir/.api-tester-patch-sha256"
mkdir -p "$vendor_root"
rm -rf "$vendor_dir"
